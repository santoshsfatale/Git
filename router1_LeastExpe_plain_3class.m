function [N_min,N_max]=router1_LeastExpe_plain_3class(produ,t_inst,ProbForSavingR1,N_min,N_max)
global memoryR1_LeastExpe Probability_producers RemovedProducer Freshness_requirment Router1_hit_count count1 count2 RemovedProducerIndexIndicator%memoryR1_LRU memoryR1_Random;
...
%         index
count2=count2+1; % Current request number
...
%     temp1=sum(produ>cumsum([0 Pop_producers])); % gives type of producer: class1/class2/class3
    if (t_inst-memoryR1_LeastExpe(index,2))<=Freshness_requirment(produ)
...
%     display('producer not present at R1')
    ...
    if count1>length(memoryR1_LeastExpe)
        [Value,index2]=FindLeastExpe_3class(memoryR1_LeastExpe(:,1),memoryR1_LeastExpe(:,2),t_inst);
        
        if (Probability_producers(produ)*Freshness_requirment(produ))>Value % Do Caching
            RemovedProducer(count2)=memoryR1_LeastExpe(index2,1);
            if memoryR1_LeastExpe(index2,1)>=1 && memoryR1_LeastExpe(index2,1)<=10
                RemovedProducerIndexIndicator(count2)=1;
            elseif memoryR1_LeastExpe(index2,1)>=11 && memoryR1_LeastExpe(index2,1)<=15
                RemovedProducerIndexIndicator(count2)=2;
            elseif memoryR1_LeastExpe(index2,1)>=16 && memoryR1_LeastExpe(index2,1)<=20
                RemovedProducerIndexIndicator(count2)=3;
            else
                RemovedProducerIndexIndicator(count2)=4;
            end
        else % No Caching
            index2=0;
        end
