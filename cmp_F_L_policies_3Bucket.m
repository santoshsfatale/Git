...
hit_rate_total_Sim_Uni_LeastExpeModified=zeros(NumberOfIterations,length(F_a));
...
hit_rate_total_Sim_Zipf_LeastExpe=zeros(NumberOfIterations,length(F_a));
hit_rate_total_Sim_Zipf_LeastExpeModified=zeros(NumberOfIterations,length(F_a));
hit_rate_total_Sim_Zipf_SMP=zeros(NumberOfIterations,length(F_a));
hit_rate_total_Sim_Zipf_RAND=zeros(NumberOfIterations,length(F_a));
hit_rate_total_Sim_Zipf_LRU=zeros(NumberOfIterations,length(F_a));
...
RemainingProbability=(1-sum(ProducersProbability_Uni(1:RouterStorageCapacity,1)))/(Producers-RouterStorageCapacity);
ProducersProbability_UniModified(:,1)=[ProducersProbability_Uni(1:RouterStorageCapacity,1);ones(Producers-RouterStorageCapacity,1)*RemainingProbability];
display(sprintf('sum(ProducersProbability_UniModified)=%f',sum(ProducersProbability_UniModified)));
Freshness_Uni=[repmat(F_a,N_a,1);repmat(F_b,N_b,length(F_a));repmat(F_c,N_c,1)];
...
%     display('Done Uniform');
    ...
    R1_hit_count_Uni_LeastExpeModified=zeros(Producers,length(F_a));
    Probability_producers(1,:)=ProducersProbability_UniModified(:,1);
    
    N_min_3class_LeastExpeModified=zeros(length(CacheSize),length(F_a));
    N_max_3class_LeastExpeModified=zeros(length(CacheSize),length(F_a));
...
saveFigAs=sprintf('Hit_rate_Vs_F_a_policies_Uniform_%d',Factor*10);
xlim1=[min(F_a/F_b) max(F_a/F_b)];
ylim1=[0 1];
...
RemainingProbability=(1-sum(ProducersProbability_Zipf(1,1:RouterStorageCapacity)))/(Producers-RouterStorageCapacity);
ProducersProbability_ZipfModified(1,:)=[ProducersProbability_Zipf(1,1:RouterStorageCapacity) ones(1,Producers-RouterStorageCapacity)*RemainingProbability];
...
N_b=CacheSize;
N_c=Producers-N_a-N_b;
Freshness_Zipf=[repmat(F_a,N_a,1);repmat(F_b,N_b,length(F_a));repmat(F_c,N_c,1)];
...
%     clear temp1 temp2
    ...
    %% Least ExpectedModified Variables :::::::::::::::::::::::::::::::::::::::::::::::
    R1_hit_count_Zipf_LeastExpeModified=zeros(Producers,length(F_a));

    N_min_Zipf_LeastExpeModified=zeros(length(CacheSize),length(F_a));
    N_max_Zipf_LeastExpeModified=zeros(length(CacheSize),length(F_a));
    ...
    for nn=1:length(F_a)
        ...
        for cache=1:length(CacheSize)
            ...
            Probability_producers(1,:)=ProducersProbability_ZipfModified(1,:);
...
directory='D:\IoT\IoT\31Jan\LeastExpected\CheckCodes\Results_F_b_400';
legend1={sprintf('LU'),sprintf('LU(modified)'),sprintf('LRU'),sprintf('RAND'),sprintf('SMP')};
saveFigAs=sprintf('Hit_rate_Vs_F_a_policies_Zipf_%d',Factor*10);
xlim1=[min(F_a/F_b) max(F_a/F_b)];
ylim1=[0 1];
myplotNew(xinput,yinputMatrix_avg,yinputMatrix_stdDev,xlabel1,ylabel1,legend1,xlim1,ylim1,saveFigAs,directory);
cd(temp1);
...
save(sprintf('cmp_F_a_policies_3Class_%d',Factor*10));
toc
%% Result Plot
