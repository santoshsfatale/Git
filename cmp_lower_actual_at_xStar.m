...
%% Optimal X calculation
optimalX=floor(CacheSize_C./(DataSize_D+1));
...
R1_hit_count_Zipf_LeastExpe=zeros(NumberOfProducers_N,length(DataSize_D));
...
%     N_max=0;
    ...
    RemainingProbability=(1-sum(ProducersProbability_Zipf(1:optimalX(dd),1)))/(NumberOfProducers_N-optimalX(dd));
    Probability_producers(:,1)=[ProducersProbability_Zipf(1:optimalX(dd),1); RemainingProbability*ones(NumberOfProducers_N-optimalX(dd),1)];
    display(sprintf('Optimal X for DataSize=%d is %d',DataSize_D(dd),optimalX(dd)));
    ...
    memoryR1_LeastExpe=zeros(optimalX(dd),2);zeros(floor((CacheSize_C(dd)-optimalX(dd))/DataSize_D(dd)),2);
...
%                     end
    ...
    R1_hit_count_Zipf_LeastExpe(:,dd)=Router1_hit_count;
    
    N_min_Zipf_LeastExpe(1,dd)=N_min;
    N_max_Zipf_LeastExpe(1,dd)=N_max;
    toc
 end
hit_rate_total_Sim_Zipf_LeastExpe(:,:)=sum(R1_hit_count_Zipf_LeastExpe)/NumberOfRequests; % Columns represents hit rate for different data size (CacheSize is PROPORTIONAL to DataSize)
...
temp1=cd;
xinput(:,1)=DataSize_D;
yinputMatrix=[hit_rate_total_Sim_Zipf_LeastExpe' LowerBound_LeastExpected'];
