...
hit_rate_total_Sim_Uni_LeastExpeModified=zeros(NumberOfIterations,length(Prob_a));
...
RemainingProbability=(1-(sum(ProducersProbability_Uni(1:RouterStorageCapacity,:))))/(Producers-RouterStorageCapacity);
ProducersProbability_UniModified=[ProducersProbability_Uni(1:RouterStorageCapacity,:);repmat(RemainingProbability,Producers-RouterStorageCapacity,1)];
...
%     display('Done Uniform');
    ...
    R1_hit_count_Uni_LeastExpeModified=zeros(Producers,length(Prob_a));

    N_min_3class_LeastExpeModified=zeros(length(CacheSize),length(Prob_a));
    N_max_3class_LeastExpeModified=zeros(length(CacheSize),length(Prob_a));

    %% 3 class Uniform Distribution Least Expectedodified

    for nn=1:length(Prob_a)
        N_min=zeros(length(ProbForSavingVectorR1),length(CacheSize));
        N_max=zeros(length(ProbForSavingVectorR1),length(CacheSize));
        for cache=1:length(CacheSize)            
            Probability_producers(1,:)=ProducersProbability_UniModified(:,nn);
            N_a=CacheSize(cache);
            N_b=CacheSize(cache);
            N_c=Producers-N_a-N_b;
            Pop_producers=[N_a N_b N_c];
...
%                 display('3 class Uniform Distribution');
    ...
    %                 Frshness=Freshness(t_inst,1);
            ...
            R1_hit_count_Uni_LeastExpeModified(:,nn)=Router1_hit_count;
        end
...
saveFigAs=sprintf('Hit_rate_Vs_Prob_a_policies_Uniform_%d',Factor*10);
...
% display('Standard Deviation for 3-class Model for Prob_A');
...
RemainingProbability=(1-sum(ProducersProbability_Zipf(:,1:RouterStorageCapacity),2))/(Producers-RouterStorageCapacity);
ProducersProbability_ZipfModified=[ProducersProbability_Zipf(:,1:RouterStorageCapacity) repmat(RemainingProbability,1,Producers-RouterStorageCapacity)];
...
N_b=CacheSize;
N_c=Producers-N_a-N_b;
Freshness_Zipf=[repmat(F_a,N_a,length(beta));repmat(F_b,N_b,length(beta));repmat(F_c,N_c,length(beta))];
% for ii=1:length(beta)
%     Freshness_Zipf(:,ii)=sqrt((const*ProducersProbability_Zipf(ii,1)./ProducersProbability_Zipf(ii,:)'));
% end

hit_rate_total_Sim_Zipf_LeastExpe=zeros(NumberOfIterations,length(Prob_a));
hit_rate_total_Sim_Zipf_LeastExpeModified=zeros(NumberOfIterations,length(Prob_a));
hit_rate_total_Sim_Zipf_SMP=zeros(NumberOfIterations,length(Prob_a));
hit_rate_total_Sim_Zipf_RAND=zeros(NumberOfIterations,length(Prob_a));
hit_rate_total_Sim_Zipf_LRU=zeros(NumberOfIterations,length(Prob_a));

for kk=1:NumberOfIterations
    display(sprintf('Iteration Number=%d',kk));
    %% Exponential inter-arrival time
    time=cumsum(exprnd(1,NumberOfRequests,1));
    producersRequest_Zipf=zeros(NumberOfRequests,length(beta));

    for jj=1:length(beta)
        producersRequest_Zipf(:,jj) = datasample(1:Producers,NumberOfRequests,'Weights',ProducersProbability_Zipf(jj,:)');  
    end
...
%     clear temp1 temp2
    hit_rate_total_Sim_Zipf_LeastExpe(kk,:)=sum(R1_hit_count_Zipf_LeastExpe)/NumberOfRequests;
    
    
    %% Least ExpectedModified Variables :::::::::::::::::::::::::::::::::::::::::::::::
    R1_hit_count_Zipf_LeastExpeModified=zeros(Producers,length(beta));

    N_min_Zipf_LeastExpeModified=zeros(length(CacheSize),length(beta));
    N_max_Zipf_LeastExpeModified=zeros(length(CacheSize),length(beta));

    %% Zipf distribution with parameter beta Least ExpectedModified
    for nn=1:length(beta)
        N_min=zeros(length(ProbForSavingVectorR1),length(CacheSize));
        N_max=zeros(length(ProbForSavingVectorR1),length(CacheSize));
        Probability_producers(1,:)=ProducersProbability_ZipfModified(nn,:);
...
%                 display('Zipf Distribution');
        ...
        %                 Frshness=Freshness(t_inst,1);
            ...
            R1_hit_count_Zipf_LeastExpeModified(:,nn)=Router1_hit_count;
        end
        N_min_Zipf_LeastExpeModified(:,nn)=N_min;
        N_max_Zipf_LeastExpeModified(:,nn)=N_max;
    end
    toc
%     display('Done Zipf');
...
%     hit_rate_Simul_Zipf_LeastExpe=R1_hit_count_Zipf_LeastExpe./requests_Zipf;
%     clear temp1 temp2
    hit_rate_total_Sim_Zipf_LeastExpeModified(kk,:)=sum(R1_hit_count_Zipf_LeastExpeModified)/NumberOfRequests;
...
%     clear temp1 temp2
    hit_rate_total_Sim_Zipf_SMP(kk,:)=sum(R1_hit_count_Zipf_SMP)/NumberOfRequests;
...
%     clear temp1 temp2
    hit_rate_total_Sim_Zipf_RAND(kk,:)=sum(R1_hit_count_Zipf_RAND)/NumberOfRequests;
...
%     clear temp1 temp2
    hit_rate_total_Sim_Zipf_LRU(kk,:)=sum(R1_hit_count_Zipf_LRU)/NumberOfRequests;
...
clear R1_hit_count_Zipf_LeastExpe R1_hit_count_Zipf_LRU R1_hit_count_Zipf_RAND R1_hit_count_Zipf_SMP
...
temp1=cd;
xinput(:,1)=beta;
yinputMatrix_avg=horzcat(hit_rate_total_Sim_Zipf_LeastExpe_Average',hit_rate_total_Sim_Zipf_LeastExpeModified_Average',hit_rate_total_Sim_Zipf_LRU_Average',hit_rate_total_Sim_Zipf_RAND_Average',hit_rate_total_Sim_Zipf_SMP_Average');
...
directory='D:\IoT\IoT\31Jan\LeastExpected\CheckCodes\Results_F_b_400';
legend1={sprintf('LU'),sprintf('LU(modified)'),sprintf('LRU'),sprintf('RAND'),sprintf('SMP')};
saveFigAs=sprintf('Hit_rate_Vs_beta_policies_Zipf_%d',Factor*10);
...
% yinputMatrix_stdDev=horzcat(hit_rate_total_Sim_Zipf_LeastExpe_stdDev',hit_rate_total_Sim_Zipf_LRU_stdDev',hit_rate_total_Sim_Zipf_RAND_stdDev',hit_rate_total_Sim_Zipf_SMP_stdDev');
% [v, I]=max(yinputMatrix_stdDev);
% display('Standard Deviation for Zipf for beta');
% (v./[yinputMatrix_avg(I(1),1) yinputMatrix_avg(I(2),2) yinputMatrix_avg(I(3),3) yinputMatrix_avg(I(4),4)])*100
% clear yinputMatrix_avg yinputMatrix_stdDev

%% always change the dataname for saving. Keep it simple and discriptive.
% temp1=cd;
cd('D:\IoT\IoT\31Jan\LeastExpected\CheckCodes\Results_F_b_400')
save(sprintf('cmp_beta_policies_3Class_Zipf_%d',Factor*10));
toc
