function [N_min,N_max]=router1_LRU_plain_3class(produ,t_inst,ProbForSavingR1,N_min,N_max)
...
%     temp1=sum(produ>cumsum([0 Pop_producers]));
    if (t_inst-memoryR1_LRU(index,3))<=Freshness_requirment(produ)
...
% Genrate choice variable according to probabilty ProbForSaving
    ...
    %             choice
    if choice==1
        memoryR1_LRU(length(memoryR1_LRU),:)=[t_inst,produ,t_stamp];
    end
